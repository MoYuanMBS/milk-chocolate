...
class fileSyncTool():
    def __init__(self,original_folder_path:str,target_folder_path:str) -> None:
        ...
        self.__allowed_regex_pattern: re.Pattern = re.compile(r"^[\w\u4e00-\u9fff.*?]+$", re.UNICODE)
    ...
    def _get_files_and_pathes(self):
        self.__original_folder.get_file_and_dir_path(self.__ignored_files_regex, self.__ignored_directories_regex, self.__ignored_type)
        self.__target_folder.get_file_and_dir_path(self.__ignored_files_regex,self.__ignored_directories_regex, self.__ignored_type)
        return 
    
    def __compare_dir_path_difference(self,first_compare_path:set[Path],second_compare_path:set[Path])->set[Path]:
        '''
    ...
    def __compare_file_path_difference(self,first_compare_path:dict[Path,float],second_compare_path:dict[Path,float])->set[Path]:
        '''
        比较原始文件和目标文件中的文件
        返回缺失的文件路径
        '''
    ...
    def __copy_file_with_chunk(self, organinal_file_path: Path, target_file_path: Path, print_ticket:int):
    ...
    def _copy_files(self):
        ...
        仅复制缺失的文件到目标文件夹
        '''
        files_need_to_copy = self.__compare_file_path_difference(self.__original_folder.full_path_with_files, self.__target_folder.full_path_with_files)
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for file in files_need_to_copy:
                ...
                future = executor.submit(self.__copy_file_with_chunk, original_file_path, target_file_path, 3)
    ...
    def _update_changed_files(self):
        ...
        with ThreadPoolExecutor() as executor:
            futures = []
            # 更新已更改的文件
            for files in self.__original_folder.full_path_with_files.keys() & self.__target_folder.full_path_with_files.keys():
                if self.__original_folder.full_path_with_files[files] > self.__target_folder.full_path_with_files[files]:
                    original_file_path = self.__original_folder.current_directory / files
                    target_file_path = self.__target_folder.current_directory / files
                    if target_file_path.exists():
                        target_file_path.unlink()
                    future = executor.submit(self.__copy_file_with_chunk, original_file_path, target_file_path, 4)
                    futures.append(future)
    ...
    def _remove_directories(self):
        ...
        # 删除多余的目录
        for dir in self.__compare_dir_path_difference(self.__target_folder.all_directory_path, self.__original_folder.all_directory_path):
            for item in self.__original_folder.all_directory_path:
                if len(item.parts) < len(dir.parts) and item.parts == dir.parts[:len(item.parts)]:
                    dir_need_to_remove[self.__target_folder.current_directory / dir] = len(dir.parts) - len(item.parts)
                else:
                    dir_need_to_remove[self.__target_folder.current_directory / dir] = len(dir.parts)
    ...
    def advanced_file_and_directory_filtering(self, file_pattern : str = '', directory_pattern: str = '', ignored_type:bool = True):
        '''
        仅复制符合正则表达式的文件
    ...
    def ignored_files_and_directories(self, files: set[str] = set(), directories: set[str] = set(), ignoed_type:bool = True):
        '''
        忽略指定的文件名或目录名
        ...
        self.__ignored_files_regex = re.compile(files_patterns[:-1]) if files_patterns else None

        dir_patterns:str = ''
        for directory in directories:
            if directory not in erro_dir_names:
                dir_patterns += f'{re.escape(directory)}|'
        self.__ignored_directories_regex = re.compile(dir_patterns[:-1]) if dir_patterns else None
...
class filesAndDirs():
    ...
    def get_file_and_dir_path(self, files_ignored_regex:re.Pattern | None, directories_ignored_regex:re.Pattern | None, ignore_type:bool):
        temp_directory_path_set = set()
        for item in set(Path.rglob(self.current_directory,'**/*')):
            if item.is_file():
                temp_file_result = self.filtering(item.name, files_ignored_regex, ignore_type)
                temp_dir_result = self.filtering(item, directories_ignored_regex, ignore_type)
                if (temp_file_result and temp_dir_result) if ignore_type else (temp_file_result or temp_dir_result):
    ...
    def filtering(self, copmared_path: Path|str, pattern: re.Pattern | None , ignored_type: bool)-> bool:
        '''
        过滤文件和目录
